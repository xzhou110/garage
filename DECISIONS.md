...
## ADR-009 — Bring TCO in by VENDORING car-tco-compare's pure engine (not a shared package / submodule / API)
- **Decision:** Copy the pure TCO calculator from the sibling repo `car-tco-compare` (`app/src/lib/tco.ts` + `depreciation.ts` + `data/reference.ts`) into garage at `app/src/lib/tco/` (verbatim files + a garage-specific `resolve.ts` adapter), and compute TCO live from the existing `Settings.years`/`Settings.miles`. The computed TCO is the **default**; the existing manual `tco5yr` field becomes an **override** (e.g. an Edmunds/AAA figure). Wired into the TCO sort, compare table, detail view, and Sheet export.
- **Context:** User wanted the TCO calculator (with a user-chosen holding period) from `car-tco-compare` to **rank** the RAV4 shortlist here. The two apps are separate GitHub repos / separate Vite/React/TS builds. Garage already had the scaffolding (`tco5yr` field, `years`/`miles` settings, per-year/per-mile derive, a TCO sort, TCO UI rows) but `tco5yr` was `null` for every car, so the sort/rows were inert; the per-year math also divided a fixed "5-yr" number by a variable horizon (latent bug).
- **Alternatives:**
  - (a) **Shared npm package** consumed by both apps — rejected for now: publishing/versioning/CI overhead is overkill for two small personal apps and adds a release step to every engine tweak. Promote to this only if a 3rd consumer appears.
  - (b) **Git submodule / subtree** of the other repo — rejected: fiddly on Windows + Pages Actions, drags in the whole other repo, and the two apps have different `types.ts`.
  - (c) **Runtime call to the deployed car-tco app** — rejected: adds a network dependency + CORS to a static, offline-capable SPA for what is pure client-side math.
- **Rationale:** The engine is small, pure, dependency-free, and already unit-tested. Vendoring keeps garage a self-contained static SPA (GitHub Pages, no backend, works offline), adds **zero runtime deps**, and is fully reversible. The one cost — duplication across repos — is mild because the engine is stable pure math; mitigated with vendor-header provenance + `app/src/lib/tco/README.md` (re-sync instructions). Ranking value holds even with placeholder cost rates: assumptions common to every car cancel out, so the ranking is driven by what differs per car — price, age, mileage.
- **Honesty:** the reference cost rates are illustrative placeholders, so TCO is surfaced as an **"Est."** with the assumptions shown (cash basis, CA regional averages) — never asserted as authoritative.
- **Reversible?** Yes — the module is isolated under `lib/tco/`; deleting it and reverting `derive.ts`/UI labels removes the feature cleanly. Manual `tco5yr` overrides keep working regardless.
